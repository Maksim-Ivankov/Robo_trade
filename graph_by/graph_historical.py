...
def settings_window55():
    global win55
    win55 = customtkinter.CTk()
    win55.title("Robo_trade")
    w = 692
    h = 610
    ws = win55.winfo_screenwidth()
    hs = win55.winfo_screenheight()
    x = (ws/2) - (w/2)
    y = (hs/2) - (h/2)
    win55.geometry('%dx%d+%d+%d' % (w, h, x, y))    
    # выстраиваем сетку гридов
    win55.grid_rowconfigure(0, weight=1)
    win55.grid_columnconfigure(1, weight=1)

# рисуем все свечи по историческим
def paint_bar(canv,prices,prices_old,VOLUME,height_canvas,width_canvas):
    global OldRange
    global NewRange
    global price_min
    global price_max
    ...
    for index, row in prices.iterrows():
        ...
        paint_candle(canv,x0,y0,y1,high,low,height_canvas)
...
# рисуем сетку
def print_setka_from_graph(height_canvas):
    mass_setka_price = []
    digit = 1000
...
def print_tools(frame):
    ...
    img_cursor = PhotoImage(file=sys.path[0] + "\graph_by\image\png\cursor.png") 
    img_line_trend = PhotoImage(file=sys.path[0] + "\graph_by\image\png\line_trend.png") 
    img_horiz_line = PhotoImage(file=sys.path[0] + "\graph_by\image\png\horiz_line.png") 
    img_kanal = PhotoImage(file=sys.path[0] + "\graph_by\image\png\kanal.png") 
    img_kist = PhotoImage(file=sys.path[0] + "\graph_by\image\png\kist.png") 
    img_korzina = PhotoImage(file=sys.path[0] + "\graph_by\image\png\korzina.png") 
...
def draw_graph(df,VOLUME,step_input,trend,TP,SL,bg="#161A1E"):
    settings_window55()
    global canvas
    global canvas_price
    global canvas_date
    global canvas_volume
    global canvas_tools,width_telo,width_spile,flag_pricel
    ...
    frame = customtkinter.CTkFrame(win55, corner_radius=0)
    frame.pack(pady=[0,0])
    ...
    canvas.bind('<Motion>', lambda event:mmove(event,height_canvas))
    
    paint_bar(canvas,df,df,VOLUME,height_canvas,width_canvas)
    print_setka_from_graph(height_canvas)
