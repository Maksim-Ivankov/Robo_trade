...
# рисуем скользящую среднюю
def print_middle_line(n,df,canv):
    global save_middle_price
    global high_midle
    OldRange1 = (VOLUME)  
    NewRange1 = (width_canvas/(144/VOLUME))  
    ...
    for index, row in df.iterrows():
        if index == n:
            ...
            save_middle_price = sum_price/n
            x0 = ((index * NewRange1) / OldRange1)
            high_midle = (((save_middle_price - price_min) * NewRange) / OldRange)
        if index>n:
            sum_price = 0
            for i in range(index-n,index):
                sum_price=sum_price+df['close'][i]*(1-(2/(index-i+1))*0.01)
            middle_price = sum_price/n
            x = ((index * NewRange1) / OldRange1)
            y = (((middle_price - price_min) * NewRange) / OldRange)
            canv.tag_lower(canv.create_line(x0+2,height_canvas-high_midle,x+2,height_canvas-y,width=1,fill="#e9f23f"))
            high_midle = y
            x0 = x

# рисуем линию по точкам закрытия         
def print_close_line(n,df,canv):
    global save_middle_price
    global high_midle
    OldRange1 = (VOLUME)  
    NewRange1 = (width_canvas/(144/VOLUME))  
    ...
    for index, row in df.iterrows():
        ...
        if index>n:
            ...
            x = ((index * NewRange1) / OldRange1)
            y = (((middle_price - price_min) * NewRange) / OldRange)
            canv.tag_lower(canv.create_line(x0+2,height_canvas-high_midle,x+2,height_canvas-y,width=1,fill="#3fa5f2"))
            high_midle = y
            x0 = x
